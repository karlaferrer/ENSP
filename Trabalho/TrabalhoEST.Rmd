...
```{r, include = TRUE}
#create table 1 with p value 
table1(~ D4_2 + D6_2 + D8_2 + D16 | df2$estado.saude.agora, rowlabelhead = "Padrão alimentar em 2022/Saúde em 2022", data=df2, overall=F, render.missing=NULL, extra.col=list(`Valor-p`=pvalue))
```
...
```{r}
#Intervalos de confiança para os ODDS com o teste de Fisher

# D4_2 FVL e estado de saúde 2022 
fisher.test(table(df2$D4_2,df2$estado.saude.agora))

# D6_2 ultraprocessados e estado de saúde 2022 
fisher.test(table(df2$D6_2,df2$estado.saude.agora))

# D8_2 bebidas acucaradas e estado de saúde 2022 
fisher.test(table(df2$D8_2,df2$estado.saude.agora))
...
## Tabela 2 - Intervalos 579+221de confiança das razões de prevalência e das razões de chance de prevalência para variáveis do bloco D - Padrão alimentar
...
# fazer somente melhorou e piorou
mel_pio<- rbind(c(189,58),c(210,181))
# IC para RP e POR - mudança no padrao alimentar
library(epiR)
